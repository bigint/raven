...
- [ ] Verify `.gitignore` covers all sensitive files (`.env`, `.env.*`, credentials, private keys)
- [ ] Confirm `.env.example` uses only placeholder values with no real credentials
- [ ] Run a secrets scanner (e.g., `trufflehog`, `gitleaks`) against the full repo history
- [ ] Review all hardcoded URLs, IPs, and internal hostnames — replace with configurable env vars
- [ ] Check Docker entrypoint and compose files for embedded secrets
- [ ] Ensure no internal infrastructure details (VPC IDs, internal domains, database hostnames) are exposed

## Documentation

- [ ] Create `README.md` with project overview, features, architecture diagram, quick start, and badges
- [ ] Create `CONTRIBUTING.md` with contribution workflow, branch strategy, PR process, and code style expectations
- [ ] Create `CODE_OF_CONDUCT.md` (Contributor Covenant recommended)
- [ ] Create `SECURITY.md` with vulnerability disclosure policy and contact information
...
- [ ] Add `repository`, `homepage`, `bugs`, `description`, `keywords`, and `license` fields to root `package.json`
- [ ] Verify `LICENSE` file is present and correct (Apache 2.0)
...
- [ ] Create GitHub Actions workflow for CI (lint, typecheck, build) on pull requests
- [ ] Create GitHub Actions workflow for Docker image builds on release tags
...
- [ ] Create `.github/PULL_REQUEST_TEMPLATE.md` with description, testing, and checklist sections
...
- [ ] Run `pnpm typecheck` and resolve all type errors
- [ ] Run `pnpm build` and confirm all apps and packages build successfully
...
- [ ] Ensure consistent formatting across the codebase (`pnpm format`)
...
- [ ] Document required system dependencies (Node.js version, pnpm version, Docker)
- [ ] Test the onboarding flow end-to-end as a new user
- [ ] Confirm `.env.example` has every required variable with clear descriptions

## Licensing & Legal

- [ ] Confirm Apache 2.0 is the intended license for all code
...
- [ ] Confirm `bigint` or org name is correctly attributed in the `LICENSE` file
...
- [ ] Provide example deployment configs (Docker Compose for production, Kubernetes manifests if applicable)
