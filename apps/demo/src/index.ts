...
async function chatCompletion() {
  ...
  const response = await client.chat.completions.create({
    messages: [
      ...
      {
        content: "What is an AI gateway and why would I use one?",
        role: "user"
      }
    ],
    model: "gpt-4o-mini"
...
async function streamingChat() {
  ...
  const stream = await client.chat.completions.create({
    messages: [{ content: "Write a haiku about API gateways.", role: "user" }],
    model: "gpt-4o-mini",
    stream: true
  });
...
async function main() {
  ...
  try {
    ...
    console.log(
      "Check the dashboard at http://localhost:3000/requests to see the logs.\n"
    );
  } catch (err) {
    if (err instanceof OpenAI.APIError) {
      ...
      if (err.status === 404) {
        console.error(
          "Check that you have an OpenAI provider configured in the dashboard."
        );
      }
    } else {
      throw err;
