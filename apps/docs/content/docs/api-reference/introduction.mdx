...
<Mermaid chart='graph LR
  A[Your App] -->|"/v1/chat/completions"| B[Proxy API]
  C[Dashboard] -->|"/providers, /keys"| D[Management API]
  B --> E[Raven Gateway]
  D --> E
  E --> F[LLM Providers]' />
...
Two authentication methods are supported. See [Authentication](/docs/api-reference/authentication) for full details.
...
Rate limits are applied per virtual key. When exceeded, the API returns `429 Too Many Requests`. See [Rate Limiting](/docs/security/rate-limiting) for configuration details.
...
<Cards>
  ...
  <Card title="Chat Completions" href="/docs/api-reference/endpoints/chat-completions">
