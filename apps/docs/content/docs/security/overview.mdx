...
Every virtual key can have independent rate limits. See [Rate Limiting](/docs/security/rate-limiting) for details.
...
<Mermaid chart="graph TD
  A[Incoming Request]:::input --> B[TLS Termination]
  B --> C[Authentication]
  C --> D[Rate Limiting]
  D --> E[Budget Check]
  E --> F[Guardrails]
  F --> G[Provider Forwarding]:::output
...
Content is scanned for PII, prompt injection, and custom rules. See [Prompt Injection Detection](/docs/security/prompt-injection).
...
<Cards>
  <Card title="Rate Limiting" href="/docs/security/rate-limiting">
    Control request throughput per key.
  </Card>
  <Card title="Prompt Injection" href="/docs/security/prompt-injection">
