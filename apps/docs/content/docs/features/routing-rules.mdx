...
<Mermaid chart="graph TD
  A[Incoming Request]:::input --> B{Rule 1}
  B -->|Match| C[Model A]:::target
  B -->|Skip| D{Rule 2}
  D -->|Match| E[Model B]:::target
  D -->|Skip| F{Rule 3}
  F -->|Match| G[Model C]:::target
  F -->|No match| H[Original Model]:::fallback
  classDef input stroke-width:2px
  classDef target stroke-width:1px
  classDef fallback stroke-width:1px,stroke-dasharray:5 5" />
