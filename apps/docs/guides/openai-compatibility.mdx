...
Raven normalizes streaming output from all providers to the OpenAI SSE format, so even Anthropic streams arrive in the same format. See [Streaming](/guides/streaming) for details.
