...
|-----------|---------------|
| By provider | Compare spend across OpenAI, Anthropic, Google, etc. |
...
Monitor how effectively the semantic cache is reducing costs and latency:
...
<Note>
  After the retention period, detailed request logs are pruned but aggregated metrics (daily totals, cost summaries) may be retained longer for trend analysis.
</Note>
...
```bash
curl https://api.raven.dev/v1/analytics \
