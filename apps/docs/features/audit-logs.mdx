...
| `policy` | Activated compliance policy, archived old policy |
...
| `prompt` | Created prompt template, published new version |
| `team` | Created team, added member |
| `organization` | Updated org settings, changed plan |
| `member` | Invited member, changed role |
| `agent` | Registered agent identity, suspended agent |
| `mcp_server` | Registered MCP server, updated access control |
| `experiment` | Started experiment, completed experiment |
| `plugin` | Enabled plugin, updated configuration |
...
```bash
curl https://api.raven.dev/v1/admin/audit-logs \
  -H "Authorization: Bearer rk_live_abc123" \
  -G \
  -d "resourceType=virtual_key" \
  -d "action=create"
```
...
<Warning>
  Audit logs are an Enterprise feature. Contact sales to enable audit logging for your organization.
</Warning>
