...
export const createTenantQueries = (db: Database, orgId: string) => ({
  ...
  invitations: () =>
    db.select().from(invitations).where(eq(invitations.organizationId, orgId)),
  keys: () =>
    db.select().from(virtualKeys).where(eq(virtualKeys.organizationId, orgId)),
  logs: () =>
    db.select().from(requestLogs).where(eq(requestLogs.organizationId, orgId)),
  members: () =>
    db.select().from(members).where(eq(members.organizationId, orgId)),
  organization: () =>
  ...
  ssoConfig: () =>
    db.select().from(ssoConfigs).where(eq(ssoConfigs.organizationId, orgId)),
  subscriptions: () =>
    db
      .select()
      .from(subscriptions)
